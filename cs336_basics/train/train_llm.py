...
def get_args() -> argparse.Namespace:
    ...
    parser.add_argument("--dtype", type=str, default="uint16", choices=("uint16", "int32"), 
                        help="dtype for the *.bin shards")
    
    # Checkpointing
    parser.add_argument("--out_dir", type=str, default="./checkpoints")
    parser.add_argument("--resume", action="store_true", help="Resume from the most recent checkpoint in out_dir")
    parser.add_argument("--checkpoint_interval", type=int, default=1_000, help="Save a checkpoint every N")
...
def evaluate(
    ...
    for _ in range(args.eval_iters):
        ...
        logits = model(xb)  # logits: (batch_size, seq_length, vocab_len)
...
def main():
    ...
    if args.resume:
        ...
        if ckpt_path is not None:
            load_checkpoint(ckpt_path, model_cls=TransformerLM, optimizer_cls=AdamW)
        else:
            model = TransformerLM(
    ...
    for step in range(start_step, args.max_steps):
        ...
        logits = model(xb)
        loss = cross_entropy(logits.view(-1, logits.size(-1)), yb.reshape(-1))
