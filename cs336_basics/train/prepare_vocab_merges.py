...
from cs336_basics.bpe.train_bpe_orig import train_bpe
from tests.common import gpt2_bytes_to_unicode
import json


def parse_arg():
    parser = argparse.ArgumentParser(description="Train a BPE tokenizer and export files of vocab + merges.")
...
def save_vocab(
    ...
    processed_vocab = {
        bytes_to_unicode_token(token_bytes, byte_encoder):
            idx for idx, token_bytes in sorted(vocab.items(), key=lambda item: item[0])
    }
    with save_path.open("w", encoding="utf-8") as f:
        json.dump(processed_vocab, f, ensure_ascii=False)
